...
def confusion_matrix_plot_peak_v2(training, tft ,data ,dong , title  ):
    # 원본 함수 
    ...
    org_count = np.array(org_count, dtype = int)
    
    # peak processing

    peak_idx , peaks  = find_peaks(predition5, 0)
    peak_max = peaks['peak_heights'].max()
    peak_mean = np.mean(peaks['peak_heights'])
    threshold = (peak_max + peak_mean) / 2
    peak_idx , peaks  = find_peaks(predition5, threshold)
    ax0.plot(df_index.iloc[peak_idx] , predition5[peak_idx] , "x" , label = 'find peak')
    predition5[:] = 0
    predition5[peak_idx] = 1
    
    ax0.plot(df_index , predition5 , alpha= 0.6 , label = 'peak rounding ')
    print(f'{dong} peak max : {peak_max:5f} // peak_meam {peak_mean:5f} , threshlod {threshold:5f}')

    #org_count , predition5 = post_processing(org_count , predition5 , cut_off)   # cut_off 적용 코드
...
def dong_f2(training, tft ,data, dong):
    df = data[data['h_dong']==dong]
    df_index = df[df['time_idx'] > 24* 7 -1]['REG_DTIME']
    org_count = df[df['time_idx'] > 24* 7 -1 ]['count']
...
def dong_f2_all(training , tft ,data):
    ...
    peak_idx , peaks  = find_peaks(predictions.reshape(-1), 0)
    peak_max = peaks['peak_heights'].max()
    peak_mean = np.mean(peaks['peak_heights'])
    threshold = (peak_max + peak_mean) / 2
    peak_idx , peaks  = find_peaks(predictions.reshape(-1), threshold)
