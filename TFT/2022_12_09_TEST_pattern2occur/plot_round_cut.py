...
def confusion_matrix_plot_round_cut(training, tft ,data, cut_off ,dong , title , round_cut):
    # 원본 함수 
    ...
    predition5 = np.logical_not(predition5 < round_cut)
...
def dong_f2(training, tft ,data,cut_off, dong, round_cut : float):
    df = data[data['h_dong']==dong]
    df_index = df[df['time_idx'] > 24* 7 -1]['REG_DTIME']
    org_count = df[df['time_idx'] > 24* 7 -1 ]['count']
...
def dong_f2_test(training, tft ,data, dong, cut_off,round_cut):
    df = data[data['h_dong']==dong]
    df_index = df[df['time_idx'] > 24* 7 -1]['REG_DTIME']
    org_count = df[df['time_idx'] > 24* 7 -1 ]['count']
...
def dong_f2_all(training , tft ,data,cut_off , round_cut):
    ...
    for i in range(2,52):
        val_data = get_val_dataloader(training ,data, 24*7 + 24*i)
        pred , x, idx_df = tft.predict(val_data, mode='raw', return_x = True , return_index = True)
        predictions = torch.cat([predictions ,pred['prediction'][:, : , 3]], dim=1) 
    ...
    predictions = np.logical_not(predictions < round_cut)
    org_count , predition5 = post_processing(org_count , predictions , cut_off)
    return metrics.fbeta_score(org_count,predictions , 2)    
    
def get_f2(training, tft ,data,cut_off, round_cut):
    ...
    for idx, dong in enumerate(dongs):
        ...
        f2_df.loc[idx,'f2_score'] = dong_f2(training, tft ,data,cut_off, dong ,round_cut)
