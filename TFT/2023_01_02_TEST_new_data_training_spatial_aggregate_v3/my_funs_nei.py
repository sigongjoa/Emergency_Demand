...
def moving_average_alpha_both(df: pd.DataFrame, unit: float):
    ...
    for dong in df['h_dong'].unique():
        dong_df = df[df['h_dong'] == dong]
        max_value = dong_df['nei_count'].max()
        nei_back_ewma = dong_df['nei_count'].ewm(alpha = unit).mean()

        inv_dong_df = dong_df[::-1]
        nei_for_ewma = inv_dong_df['nei_count'].ewm(alpha = unit).mean()
        
        nei_ewma = (nei_for_ewma + nei_back_ewma) / 2
        nei_ewma = nei_ewma / nei_ewma.max() * max_value
        df['nei_count'][dong_df.index] = nei_ewma 
...
def graph_count(graph_data : pd.DataFrame, w1 : float , w2 : float , w3 : float):
    ...
    graph_data['nei_count'] = graph_data['nei1'] 
    return graph_data
...
def data_processing(path : str  , unit : float , ewma_fun : moving_average_alpha ):
    ...
    # 사용하는 column만 선택
    data = data[['REG_DTIME', 'h_dong', 'count', 'pops', 'windspd','humid', 'temp', 'precip_form', 'precip', 'isHoliday' , 'nei1', 'nei_count']]
