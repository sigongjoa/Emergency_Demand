...
def data_processing(path : str , unit : int):
...
def get_training(data , max_prediction_length, max_encoder_length):
    ...
    training = TimeSeriesDataSet(
        ...
        time_varying_known_categoricals=["HOD", "DOW" , 'MOY'],
...
def countious_prediction(training, tft ,ewma_data, dong, title):
    ...
    for i in range(1,59):
        val_data = get_val_dataloader(training ,ewma_data, 96408+24*i)
    ...
    ewma_df_index = ewma_dong_df[ewma_dong_df['time_idx'] > 96408-1]['REG_DTIME']
    ewma_org_count = ewma_dong_df[ewma_dong_df['time_idx'] > 96408-1]['count']
    
    org_df = data_processing('../../data/data.csv')
    dong_org_df = org_df[org_df['h_dong'] == dong]
    dong_org_count = dong_org_df[dong_org_df['time_idx'] > 96408-1]['count']
